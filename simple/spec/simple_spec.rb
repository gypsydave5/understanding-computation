...
describe 'SIMPLE' do
  ...
  it 'can add' do
    ...
    expect(machine.run.statement).to eql Number.new(3)
  end

  it 'can multiply' do
    expression = Multiply.new(Number.new(2), Number.new(2))
    machine = Machine.new(expression)
    expect(machine.run.statement).to eql Number.new(4)
  end

  it 'can subtract' do
    expression = Subtract.new(Number.new(4), Number.new(3))
    machine = Machine.new(expression)
    expect(machine.run.statement).to eql Number.new(1)
  end
  ...
  it 'can do assignment' do
    ...
    expect(machine.run.environment[:x]).to eql Number.new(3)
  end
